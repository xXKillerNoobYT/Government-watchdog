"""Tests for GOV-195 batch-4 promotion script.
...
import db  # noqa: E402
import gov195_promotion_batch4 as promo  # noqa: E402
...
def _seed_all_batch4(conn: sqlite3.Connection) -> None:
    for sid in promo.BATCH4_STATEMENT_IDS:
...
class TestPreflight:
    def test_preflight_passes_with_all_rows(self, conn: sqlite3.Connection) -> None:
        _seed_all_batch4(conn)
        plan = promo._preflight(conn)
        assert len(plan) == len(promo.BATCH4_STATEMENT_IDS)

    def test_preflight_rejects_missing_row(self, conn: sqlite3.Connection) -> None:
        for sid in promo.BATCH4_STATEMENT_IDS[1:]:
...
class TestPromotion:
    ...
    def test_apply_promotes_all_22(self, conn: sqlite3.Connection, tmp_path: Path) -> None:
        _seed_all_batch4(conn)
...
class TestInvariants:
    def test_no_duplicate_ids(self) -> None:
        assert len(set(promo.BATCH4_STATEMENT_IDS)) == len(promo.BATCH4_STATEMENT_IDS)

    def test_all_ids_are_ai_corpus_pattern(self) -> None:
        for sid in promo.BATCH4_STATEMENT_IDS:
            assert sid.startswith("alpine_local_corpus:ai:")

    def test_batch_count_is_22(self) -> None:
        assert len(promo.BATCH4_STATEMENT_IDS) == 22
    ...
    def test_no_overlap_with_prior_batches(self) -> None:
        ...
        batch4_set = set(promo.BATCH4_STATEMENT_IDS)
        batch1_set = set(b1.CONFIRMED_STATEMENT_IDS)
        batch2_set = set(b2.BATCH2_STATEMENT_IDS)
        batch3_set = set(b3.BATCH3_STATEMENT_IDS)
        assert not batch4_set & batch1_set, f"overlap with batch-1: {batch4_set & batch1_set}"
        assert not batch4_set & batch2_set, f"overlap with batch-2: {batch4_set & batch2_set}"
        assert not batch4_set & batch3_set, f"overlap with batch-3: {batch4_set & batch3_set}"
