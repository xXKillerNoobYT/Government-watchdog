"""Tests for GOV-200 batch-5 promotion script.
...
import db  # noqa: E402
import gov200_promotion_batch5 as promo  # noqa: E402
...
def _seed_all_batch5(conn: sqlite3.Connection) -> None:
    for sid in promo.BATCH5_STATEMENT_IDS:
...
class TestPreflight:
    def test_preflight_passes_with_all_rows(self, conn: sqlite3.Connection) -> None:
        _seed_all_batch5(conn)
        plan = promo._preflight(conn)
        assert len(plan) == len(promo.BATCH5_STATEMENT_IDS)

    def test_preflight_rejects_missing_row(self, conn: sqlite3.Connection) -> None:
        for sid in promo.BATCH5_STATEMENT_IDS[1:]:
...
class TestInvariants:
    def test_no_duplicate_ids(self) -> None:
        assert len(set(promo.BATCH5_STATEMENT_IDS)) == len(promo.BATCH5_STATEMENT_IDS)

    def test_all_ids_are_ai_corpus_pattern(self) -> None:
        for sid in promo.BATCH5_STATEMENT_IDS:
            assert sid.startswith("alpine_local_corpus:ai:")

    def test_batch_count_is_19(self) -> None:
        assert len(promo.BATCH5_STATEMENT_IDS) == 19

    def test_no_overlap_with_prior_batches(self) -> None:
        import gov146_promotion_seed as b1
        import gov162_promotion_batch2 as b2
        import gov192_promotion_batch3 as b3
        import gov195_promotion_batch4 as b4
        batch5_set = set(promo.BATCH5_STATEMENT_IDS)
        batch1_set = set(b1.CONFIRMED_STATEMENT_IDS)
        batch2_set = set(b2.BATCH2_STATEMENT_IDS)
        batch3_set = set(b3.BATCH3_STATEMENT_IDS)
        batch4_set = set(b4.BATCH4_STATEMENT_IDS)
        assert not batch5_set & batch1_set, f"overlap with batch-1: {batch5_set & batch1_set}"
        assert not batch5_set & batch2_set, f"overlap with batch-2: {batch5_set & batch2_set}"
        assert not batch5_set & batch3_set, f"overlap with batch-3: {batch5_set & batch3_set}"
        assert not batch5_set & batch4_set, f"overlap with batch-4: {batch5_set & batch4_set}"
