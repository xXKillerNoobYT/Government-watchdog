"""GOV-428 Stage 3.15 — agent-handoff / owner-escalation routing guard.

Proves :mod:`stage3_escalation` enforces that every Stage-3 reviewer-internal
fail-closed / unresolvable condition class terminates at exactly one named human/governance
...
the SSOT manifest, then for each check injects a routing defect into a *copy* of the
...
Test-only / read-only: imports the existing ``stage3_escalation`` functions, adds NO
...
import stage3_escalation as esc  # noqa: E402

# The full enumerated Stage-3 condition set this slice routes.
_EXPECTED_CLASS_COUNT = 11
...
def test_every_enumerated_class_has_exactly_one_route() -> None:
    ...
    assert report["clean"]
    assert report["mapped_count"] == report["enumerated_count"] == _EXPECTED_CLASS_COUNT
...
def test_carried_and_renamed_class_vocabulary() -> None:
    """Locks the carried-vs-renamed decision (module docstring).

    Three Stage-2 classes carry forward verbatim; the four Stage-2 auditor classes are
    RENAMED to their Stage-3 names — never duplicated — and the Stage-3-specific surface
    classes are present. Guards against a future edit that silently re-adds a Stage-2
    auditor name alongside its Stage-3 rename (two routes for one abstract condition).
    """
    carried = {"reviewer_access_denied", "provenance_unverified", "completeness_gap_unresolved"}
    stage3_specific = {
        "source_inventory_gap", "preservation_hash_mismatch", "card_assembly_failclosed",
        "verify_at_source_unresolved", "stage3_integration_safetynet_trip",
        "stage3_traceability_drift", "stage3_coverage_backgap", "stage3_docdrift_red",
    }
    assert carried <= esc.CONDITION_CLASSES
    assert stage3_specific <= esc.CONDITION_CLASSES
    assert carried | stage3_specific == set(esc.CONDITION_CLASSES)
    # The superseded Stage-2 auditor names must NOT also be enumerated (no double-route).
    superseded_stage2_names = {
        "coverage_backgap", "integration_safetynet_trip", "traceability_orphan", "docdrift_red",
    }
    assert superseded_stage2_names.isdisjoint(esc.CONDITION_CLASSES)
...
def test_route_for_returns_ssot_route_for_mapped_class() -> None:
    route = esc.route_for("preservation_hash_mismatch")
...
# ===========================================================================
# RED — injected routing defects flip the guard. Each is proven load-bearing:
# FAILS when the defect is present and PASSES when it is removed.
# ===========================================================================
...
def test_red_empty_owner_trips_check2(manifest) -> None:
    ...
    manifest["stage3_traceability_drift"]["owner_role"] = ""
    report = esc.named_human_owner(manifest)
    assert not report["clean"]
    assert any(v["condition_class"] == "stage3_traceability_drift" for v in report["violations"])
...
def test_red_non_human_owner_trips_check2(manifest) -> None:
    ...
    manifest["stage3_traceability_drift"]["owner_role"] = "some-random-agent"
...
def test_red_stage3_column_name_in_route_trips_check4(manifest) -> None:
    """RED: a Stage-3-specific column name (record payload) in a route trips no-leak.

    Proves the Stage-3 additions to the internal-column token set are load-bearing, not
    just carried Stage-2 tokens.
    """
    manifest["preservation_hash_mismatch"]["escalation_action"] = "re-check the manifest_digest value"
    report = esc.no_leak(manifest)
    assert not report["clean"]
    assert any("manifest_digest" in l["reason"] for l in report["leaks"])
...
def test_cli_exit_1_on_injected_defect(monkeypatch) -> None:
    ...
    broken["stage3_coverage_backgap"]["owner_role"] = broken["stage3_coverage_backgap"]["detected_by"]
