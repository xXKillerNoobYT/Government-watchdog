...
| Branch delete fails (`-d` safety) | `remove_branch` deletes with `git branch -d` for the ancestor / no-commits-ahead gate-2 paths — `-d` refuses an unmerged tip, keeping the second safety layer for the non-squash case. **Squash-only branches (GOV-1650 fix):** because every PR here squash-merges, a squash-merged branch tip is *never* an ancestor of `origin/main`, so `-d` would false-negative even when gate 2 proved containment via `git cherry` (0 `+`). When — and only when — gate 2 passed via the GOV-537 proven-containment path (`GateResult.proven_contained=True`), `remove_branch` uses `git branch -D`: containment is already proven commit-by-commit by patch-id, so the ancestry check is the wrong guard. A branch whose containment is *not* proven (`proven_contained=False`, e.g. an uncontained WIP sibling sharing the issue id) is never force-deleted — it fails gate 2 and is preserved. If `-d`/`-D` still fails: logged, no retry, investigate manually. |
