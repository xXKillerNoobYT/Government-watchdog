"""Stage 3 agent-handoff / owner-escalation routing guard (GOV-428, Stage 3.15).

Owner: AutomationOpsEngineer. Parent: GOV-405 (CTO Stage 3 sequencing). The **final
buildable Stage-3 slice**. Read-only, deterministic, Alpine-only, governance artifact —
...
--------------------------------------------------------
The Stage-3 auditors plus the card / verify-at-source read surface are all
**fail-closed**: when a condition trips, the reviewer-internal surface silently serves
*less*.

* source/data inventory (GOV-364, :mod:`stage3_source_inventory`) — every Alpine source
  accounted for; a gap means a source is missing from the inventory;
* raw-preservation (GOV-367, :mod:`stage3_preservation_audit`) — ``hash_ok`` per unit /
  replay; a mismatch means a preserved raw unit no longer matches its recorded digest;
* card-feed (GOV-347, :mod:`stage3_card_feed`) — the reviewer card surface assembles only
  fully-grounded cards; assembly fail-closed when a card cannot be safely composed;
* verify-at-source (GOV-376) — both-legs resolvability; unresolved when a source link
  cannot be verified to resolve;
* composition / integration safety-net (GOV-393) — overlays co-present, no cross-lane
  leak; a trip is a compose failure / potential cross-lane leak;
* traceability + audit-trail (GOV-406, :mod:`stage3_traceability`) — no orphan / drift;
* back-gap / coverage-regression (GOV-411, :mod:`stage3_backgap`) — nothing silently
  dropped between runs;
* doc-drift (GOV-421) — the reviewer reference doc matches the live Stage-3 surface.

None of them assert *what happens to the human* when a condition is **unresolvable**.
3.15 closes that gap exactly as Stage 2.15 (GOV-332, :mod:`stage2_escalation`) did: a
deterministic SSOT routing manifest + guard proving every Stage-3 fail-closed /
unresolvable condition **terminates at exactly one named human/governance owner with a
defined escalation action** — and never loops back to a *detecting* agent (the
anti-infinite-loop invariant). The manifest routes by abstract **condition class**, never
by record content, so it inherently carries no PII and no record payload.

Carried vs renamed from Stage 2
-------------------------------
Three Stage-2 condition classes are cross-cutting and still live unchanged at Stage 3, so
they carry forward verbatim: ``reviewer_access_denied`` (the gated-beta access gate),
``provenance_unverified`` (the GOV-311 provenance projection), and
``completeness_gap_unresolved`` (the GOV-298 completeness-gap card, now surfaced through
the 3.05 card feed). The four Stage-2 *auditor* conditions are **renamed**, not
duplicated: each Stage-3 auditor supersedes its Stage-2 analog over the live Stage-3
surface, so ``coverage_backgap`` → ``stage3_coverage_backgap`` (GOV-411),
``integration_safetynet_trip`` → ``stage3_integration_safetynet_trip`` (GOV-393),
``traceability_orphan`` → ``stage3_traceability_drift`` (GOV-406), and ``docdrift_red``
→ ``stage3_docdrift_red`` (GOV-421). Listing both the Stage-2 and Stage-3 name for the
same abstract condition would create two routes for one condition and muddy completeness.

Hard constraints (GOV-428): NO migration, NO mutation, NO AI, NO network, NO public
...
condition. NON-unlock; premium-gate N/A by construction (governance artifact, no served
field, no public projection). Subgoal ``c4edbe3f`` flips ``achieved`` only at CTO merge.
...
If implementing 3.15 ever surfaces a genuine need to touch ``read_api.py`` /
``publication.py`` (a production serving change), or a condition class has no defensible
human owner without an Isaac/owner decision, STOP and escalate (GOV-428 / GOV-332 pass-up
trigger) — never invent an owner or patch production silently.

Usage:
    python scripts/stage3_escalation.py [--json]
"""
...
# The exact, frozen set of Stage-3 reviewer-internal fail-closed / unresolvable
# condition classes. Mirrors the proven Stage-3 read surface (each cites the auditor /
# slice that raises it). Three classes carry forward unchanged from Stage 2 (cross-cutting
# and still live); the rest are Stage-3-specific (the Stage-2 auditor analogs renamed —
# see module docstring). Adding a real new condition class is a deliberate edit here AND a
# new route below — never an ad-hoc string at a call site.
CONDITION_CLASSES: frozenset[str] = frozenset(
    {
        # --- carried verbatim from Stage 2 (cross-cutting, still live) ---
        ...
        "completeness_gap_unresolved",     # unresolved completeness-gap card (GOV-298, via 3.05)
        # --- Stage-3-specific (auditor / read-surface fail-closed conditions) ---
        "source_inventory_gap",            # 3.03 source/data inventory gap (GOV-364)
        "preservation_hash_mismatch",      # 3.04 hash_ok false / preservation replay fail (GOV-367)
        "card_assembly_failclosed",        # 3.05 reviewer card cannot be safely assembled (GOV-347)
        "verify_at_source_unresolved",     # 3.07 both-legs / resolvability unresolved (GOV-376)
        "stage3_integration_safetynet_trip",  # 3.10 compose fail-closed / cross-lane leak (GOV-393)
        "stage3_traceability_drift",       # 3.12 traceability / audit-trail orphan or drift (GOV-406)
        "stage3_coverage_backgap",         # 3.13 back-gap / coverage regression (GOV-411)
        "stage3_docdrift_red",             # 3.14 reviewer reference doc-drift guard RED (GOV-421)
    }
)
...
HUMAN_OWNER_ROLES: frozenset[str] = frozenset(
    {
        ...
        "security-privacy-reviewer",  # privacy/trust owner: provenance, preservation, cross-lane leak
        "isaac-owner",                # final owner decision (access policy, scope)
    }
)

# The agents/automations that DETECT a Stage-3 condition. A route that terminates at
...
# Stage-3 column names are added to the carried Stage-2 set so the no-leak check is
# genuinely Stage-3-aware. Every token is a snake_case identifier guaranteed absent from
# the manifest's own natural-language route text and class keys (verified by the GREEN
# no-leak test); the route text is written without underscores so no token false-matches.
_INTERNAL_COLUMN_TOKENS: frozenset[str] = frozenset(
    {
        ...
        # Stage-3 read-surface column / payload identifiers
        "manifest_digest", "hash_ok", "resolvability_status", "provenance_status",
        "envelope_key", "to_source_id", "reviewer_internal_records",
        "published_records", "completeness_gap_cards", "preservation_status",
    }
)
...
_FAILCLOSED_DEFAULT: dict[str, str] = {
    "owner_role": "cto",
    "escalation_action": "unmapped Stage-3 condition class — CTO triages and assigns a "
...
ROUTING_MANIFEST: dict[str, dict[str, str]] = {
    ...
    "source_inventory_gap": {
        "owner_role": "cto",
        "escalation_action": "CTO opens a scoped defect issue for the source missing from "
        "the Stage-3 inventory and assigns the inventory/ingest fix (never a self-heal "
        "from the guard)",
        "pass_up_trigger": "escalate to ceo if the gap reflects a coverage-scope decision "
        "(which Alpine sources belong in the inventory) rather than an ingest defect",
        "detected_by": "automation-ops-engineer",
    },
    "preservation_hash_mismatch": {
        "owner_role": "security-privacy-reviewer",
        "escalation_action": "SecurityPrivacy reviewer treats the failed per-unit digest / "
        "preservation replay as a chain-of-custody integrity incident and gates the "
        "affected raw unit until the mismatch is explained or the unit is re-preserved",
        "pass_up_trigger": "escalate to cto for the engineering fix once the unit is "
        "contained, and to isaac-owner if preserved raw evidence may have been altered",
        "detected_by": "automation-ops-engineer",
    },
    "card_assembly_failclosed": {
        "owner_role": "cto",
        "escalation_action": "CTO opens a scoped defect issue for the card that could not "
        "be safely assembled; the reviewer feed stays fail-closed (the card is withheld) "
        "until the assembly defect is fixed",
        "pass_up_trigger": "escalate to ceo if the withheld card reflects an editorial "
        "coverage decision rather than a serving defect",
        "detected_by": "backend-crawler-engineer",
    },
    "verify_at_source_unresolved": {
        "owner_role": "cto",
        "escalation_action": "CTO assigns the unresolved both-legs / source-link "
        "verification to the owning backend slice; the served value stays fail-closed "
        "(shown as unresolved, never as verified) until both legs resolve",
        "pass_up_trigger": "escalate to ceo if resolving the source link requires a "
        "source-scope expansion beyond Alpine",
        "detected_by": "backend-crawler-engineer",
    },
    "stage3_integration_safetynet_trip": {
        "owner_role": "security-privacy-reviewer",
    ...
    "stage3_traceability_drift": {
        "owner_role": "cto",
        "escalation_action": "CTO assigns the orphan / audit-trail drift to the owning "
        "backend slice for a scoped fix; the served value stays fail-closed until the "
        "trace is restored",
        "pass_up_trigger": "escalate to ceo only if resolving the orphan requires a "
        "source-scope expansion beyond Alpine",
        "detected_by": "automation-ops-engineer",
    },
    "stage3_coverage_backgap": {
        "owner_role": "cto",
    ...
    "stage3_docdrift_red": {
        ...
        "Stage-3 read surface (patch whichever side drifted) before the surface is trusted",
        "pass_up_trigger": "escalate to ceo if the drift reveals a contract change that "
        "alters the Stage-3 reviewer-internal scope",
        "detected_by": "automation-ops-engineer",
    },
}
...
def _format_report(report: dict[str, Any]) -> str:
    ...
    lines = [
        "Stage 3 escalation / owner-routing guard (GOV-428) — SSOT routing manifest",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Stage 3 agent-handoff / owner-escalation routing guard (read-only, "
        "no DB). GOV-428 Stage 3.15. Exits 1 on any routing defect."
    )
    parser.add_argument(
